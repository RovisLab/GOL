...
class RegularizationSamples(object):
    def __init__(self, template_objects, num_reg_samples_per_object):
        self.mu = template_objects.features + 0.
        self.sigma = 1.
...
class GeneralizationGenerator(object):
    def __init__(self, template_objects, regularization_samples, num_objects, num_gen_samples_per_object):
        ...
        # Init mean as the fixed value of the template objects
        ...
        self.sigma = np.std(self.regularization_samples.samples, 1)
    ...
    def generalization_functions(self, losses, template_objects):
        # Calculate the standard deviation of the regularization samples
        rs_std = np.std(self.regularization_samples.samples, 1)

        # Regenerate artificial samples for each object type if the loss per object type is larger than the
        # loss threshold
        for idx, loss in enumerate(losses):
            if loss > self.loss_threshold:
                print('Calculate the loss gradient')
...
class GOL(object):
    ...
    def loss(self):
        ...
        # Calculate the loss between each corresponding object in the regularization samples and generated samples
    ...
    def train_generator(self, num_epochs=1):
    ...
    def train_classifier(self):
        ...
        # Get the samples/features labels
        features, labels = self._format_dataset(self.generalization_generator.samples)

        # Train the classifier
        self.classifier.train(features, labels)

        # Test the classifier
        self.classifier.test(features, labels)
        ...
        for idx, mu in enumerate(to.features):
            ...
            plt.plot(p_x,
                     mlab.normpdf(p_x,
                                  ...
                                  self.generalization_generator.sigma[idx]),
                     color=color,
                     linewidth=1.6)
...
class Classifier(object):
    ...
    def train(self, features, labels):
        with tf.Session() as session:
            ...
            for epoch in range(self.training_epochs):
                ...
                _, c = session.run([self.optimizer, self.loss], feed_dict={self.x: features, self.y: labels})
    ...
    def test(self, features, labels):
        with tf.Session() as session:
            ...
            # Calculate accuracy
            accuracy = tf.reduce_mean(tf.cast(correct_prediction, "float"))
            print("Accuracy =", accuracy.eval({self.x: features, self.y: labels}))
