...
class GOL(object):
    ...
    def loss(self):
        print("Reg = ", self.regularization_samples.samples)
        print("Gen = ", self.generalization_generator.samples)

        L2 = 0
        for reg_sample in self.regularization_samples.samples:
            for gen_sample in self.generalization_generator.samples:
                L2 += np.sqrt((reg_sample - gen_sample) ** 2)
                #print("L2(%f - %f) = %f" % (reg_sample, gen_sample, L2))
        m = len(self.regularization_samples.samples) * len(self.generalization_generator.samples)
        L2 = (1/m) * L2
        print(L2)
        qa=np.reshape(self.regularization_samples.samples, (len(self.regularization_samples.samples), 1))
        qw = np.reshape(self.generalization_generator.samples, (len(self.generalization_generator.samples), 1))
        dist = cdist(qa, qw, metric='euclidean')
        L2 = (1 / dist.size) * dist.sum()
        print(L2)
...
def main():
    ...
    model = GOL(
        to,
        RegularizationSamples(to, num_reg_samples=3),
        GeneralizationGenerator(range=10, num_gen_samples=5)
    )
    model.train()
