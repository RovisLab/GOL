...
class GOL(Problem):
    ...
    def evaluate(self, solution):
        ...
        print('++ epoch', '%03d' % self.epoch_count, "generalization energy", "{:.4f}".format(self.gen_energy[0]))
        print('++ epoch', '%03d' % self.epoch_count, "classification accuracy", "{:.4f}".format(self.classifier.accuracy))

        # print("Generalization Energy = ", self.gen_energy[0])
        # print("Classification Accuracy = ", self.classifier.accuracy)

        x = solution.variables[:]
        solution.objectives[:] = [self.gen_energy[0], self.classifier.accuracy]
    ...
    def _plot_distributions(self, epoch_count):
        ...
        plt.ylim([0, 0.7])
        plt.title('1-D Generative One-Shot Learning - Training Epoch ' + str(epoch_count))
        ...
        if animate:
            # File path
            file_path = anim_path + '/anim_fig_' + str(epoch_count) + '.png'
...
class Classifier(object):
    def __init__(self, num_classes, range):
        # Network parameters
...
def main():
    ...
    gg = GeneralizationGenerator(os, rs, num_objects=os.num_objects, num_gen_samples_per_object=10000)
