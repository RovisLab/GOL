...
class ImageTransformer(object):
    ...
    def perspective_transform_horizontal(self, src_img, coefficient):
        if coefficient == 0:
            return src_img
        ...
        if coefficient > 0:
            ...
            warped_points = np.float32(
                [[abs(coefficient) * src_img_width / 2, -(abs(coefficient) * src_img_height / 2)],
    ...
    def perspective_transform_vertical(self, src_img, coefficient):
        if coefficient == 0:
            return src_img
        elif coefficient < -1 or coefficient > 1:
            error_pop_up()
    ...
    def motion_blur_filter(self, src_img, amplitude):
        anchor = (-1, -1)
        depth = -1
    ...
    def modify_contrast_and_brightness(self, src_img, alpha, beta):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
    ...
    def overlay_image(self, background, foreground, x_offset=0, y_offset=0):
    ...
    def add_shadow(self, src_img, show_equalized, min_color, max_color, min_area=0.3, max_area=0.9, shapes_nr=3):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
    ...
    def apply_grain_effect(self, src_img, min_noise, max_noise):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
        ...
        if src_img_channels == 1:
            ...
            grain_mask = cv2.dilate(grain_mask, element)
            grain_mask = cv2.GaussianBlur(grain_mask, (21, 21), 21, 21)
            grain_mask_channel_r, grain_mask_channel_g, grain_mask_channel_b, grain_mask_channel_a = cv2.split(
                grain_mask)
            grain_channel_r = cv2.subtract(grain_channel_r, grain_mask_channel_r)
            grain_channel_g = cv2.subtract(grain_channel_g, grain_mask_channel_g)
            grain_channel_b = cv2.subtract(grain_channel_b, grain_mask_channel_b)
    ...
    def add_salt_and_pepper_noise(self, src_img):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
    ...
    # TODO
    def apply_fish_eye_effect(self, src_img, k, center_x, center_y):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
    ...
    def apply_chromatic_abberation(self, src_img, x_shift, y_shift):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
    ...
    def apply_halo(self, src_img, amount):
        if amount % 2 == 0:
            amount += 1
    ...
    def segment_red(self, src_img, thr1, thr2, thl, rg_dif, gb_dif, br_dif):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
    ...
    def non_color_segment(self, src_img, thl, rg_dif, gb_dif, br_dif):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
        dst_mask = np.zeros((src_img_height, src_img_width), np.uint8)
        for i in range(src_img_width):
            for j in range(src_img_height):
    ...
    def segment_white(self, src_img, thl, rg_dif, gb_dif, br_dif):
        return self.non_color_segment(src_img, thl, rg_dif, gb_dif, br_dif)

    def segment_black(self, src_img, thl, rg_dif, gb_dif, br_dif):
        return self.non_color_segment(src_img, thl, rg_dif, gb_dif, br_dif)

    def randomise_colors_for_templates(self, src_img, colors_distributions, thr1, thr2, thl, rg_dif, gb_dif, br_dif):
        src_img_height, src_img_width, src_img_channels = get_img_properties(src_img)
        if src_img_channels != 4:
            error_pop_up()
            return src_img
        ...
        bgra_with_bg_img = self.overlay_image(background_img, src_img, 0, 0)
        ...
        red_mask = self.segment_red(bgr_img, thr1, thr2, thl, rg_dif, gb_dif, br_dif)
        black_mask = self.segment_black(bgr_img, thl, rg_dif, gb_dif, br_dif)
        white_mask = self.segment_white(bgr_img, thl, rg_dif, gb_dif, br_dif)
        for i in range(src_img_width):
            for j in range(src_img_height):
                if red_mask[j, i] != 0:
...
if __name__ == "__main__":
    ...
    dst_img = transformer.overlay_image(src_img, foreground, 50, 50)
